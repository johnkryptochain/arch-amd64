#!/bin/bash
JEMAOS_DUALBOOT_LABEL="JEMAOS-DUAL-BOOT"
OPENJEMA_DUALBOOT_LABEL="OJEMA-DUAL-BOOT"
CHROME_INSTALL_CMD="/usr/share/dualboot/chromeos-install.sh"
LOG_MOD=${LOG_MOD:-jemaos_dualboot}
LOG_FILE=/tmp/jemaos_dualboot.log
JEMAOS_DUALBOOT_DIR="/jemaos"
OPENJEMA_DUALBOOT_DIR="/openjema"
JEMAOS_DUALBOOT_IMG="/jemaos/jemaos_dual_boot.img"
OPENJEMA_DUALBOOT_IMG="/openjema/openjema_dual_boot.img"
VERSION="2.0.4"
JEMAOS_FINGERPRINT=".jemaos_dualboot"
# test if system in dualboot mode
is_dualboot() {
  [ -n "$(grep jemaos_dualboot /proc/cmdline)" ]
}

is_openjema() {
  cat /etc/lsb-release | grep CHROMEOS_RELEASE_BOARD | grep -qi openjema
}

get_dualboot_label() {
  if ! is_openjema; then
    echo "$JEMAOS_DUALBOOT_LABEL"
  else
    echo "$OPENJEMA_DUALBOOT_LABEL"
  fi
}

get_dualboot_dir() {
  if ! is_openjema; then
    echo "$JEMAOS_DUALBOOT_DIR"
  else
    echo "$OPENJEMA_DUALBOOT_DIR"
  fi
}

get_dualboot_img() {
  if ! is_openjema; then
    echo "$JEMAOS_DUALBOOT_IMG"
  else
    echo "$OPENJEMA_DUALBOOT_IMG"
  fi
}
...
# label: $1 like JEMAOS-ROOT-A (partition lable) device: $2 like /dev/sda
...
touch_dir() {
  if [ -d $1 ]; then
    touch $1/${JEMAOS_FINGERPRINT}
  fi
}

list_touched_dir() {
  if [ -d $1 ]; then
    for v_dir in `find $1 -name ${JEMAOS_FINGERPRINT}`; do
      echo ${v_dir%/*}
    done
  fi
}
...
create_dualboot_image() {
  ...
  if [ $imgspace -lt $((1024*1024*9)) ];then
    die "Need more free space to create openJema/JemaOS image, abort."
  fi
  if [ -f $img ];then
    rm -f $img
  fi
  info "Creating openJema/JemaOS multi-boot image..."
  truncate -s $(($imgspace*1024)) $img
  info "Allocate :${img}"
  local loopdev=$(load_img_to_dev $img)
  info "Installing openjema/JemaOS image..."
