...
print_usage() {
    print_version
    echo
    echo "Uninstall openJema/JemaOS multi-boot scheme"
    echo "Usage: $SELF [-h | --help]
       Example:
           $SELF                 #Uninstall openJema/JemaOS multi-boot on hard drive.
       "
}
...
main() {
  info "Removing openJema/JemaOS grub and rEFInd..."
  for efi_part in $(get_efi_part); do
    efi_dir=$(get_mnt_of_part $efi_part)
    for jema_dir in $(list_touched_dir $efi_dir); do
      if [ -n "$(echo $jema_dir | grep refind)" ]; then
        remove_boot_entry
      fi
      rm -rf $jema_dir
    done
  done
  info "Removing openJema/JemaOS multi-boot partition..."
