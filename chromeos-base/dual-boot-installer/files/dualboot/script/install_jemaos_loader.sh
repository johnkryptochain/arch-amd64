...
BOOT_LOADER_DIR="/usr/share/dualboot/jemaos"
BOOT_FILES="bootx64.efi os_jemaos.png"
LOG_MOD="install_jemaos_loader"
TPL="grub.cfg.tpl"
TMP_DIR="/tmp"
EFI="/EFI/jemaos"
SELF=$(basename $0)
...
. $DUAL_SCRIPT_DIR/jemaos_util.sh

if is_openjema; then
  EFI="/EFI/openjema"
  BOOT_LOADER_DIR="/usr/share/dualboot/openjema"
  BOOT_FILES="bootx64.efi os_openjema.png"
fi

print_version() {
    echo "$SELF version:${VERSION}, maintained by Jema Innovations. All rights reserved."
}

print_usage() {
    print_version
    echo
    echo "Installing openJema/JemaOS bootloader for multi-boot scheme."
...
_main() {
  ...
  info "Installing openJema/JemaOS bootloader..."
...
main() {
  if [ -z "$dual_boot_dev" ]; then
    die "openjema/JemaOS multi-boot partition not found, abort."
  fi
  while [[ $# -gt 0 ]]; do
      opt=$1
      case $opt in
