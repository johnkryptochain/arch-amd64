...
DEPEND="
  ...
  dual_boot_ramfs? ( ${JEMAOS_DEPENDS} )
  core_util_ramfs? ( ${JEMAOS_DEPENDS} sys-apps/frecon-lite virtual/udev )
...
src_prepare() {
  ...
  if ! use jemaos; then
     cp "${S}/dual_boot/openjema_dual_boot_mount.sh" "${S}/dual_boot/dual_boot_mount.sh"
  else
    cp "${S}/dual_boot/jemaos_dual_boot_mount.sh" "${S}/dual_boot/dual_boot_mount.sh"
  fi
